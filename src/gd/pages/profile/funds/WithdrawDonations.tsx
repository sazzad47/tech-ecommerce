...
import { useCreateDonationsWithdrawalRequestMutation } from "../../../../state/api/gd";
...
export default function WithdrawDonations() {
  const { access_token } = useSelector((state: RootState) => state.auth);

  const [
    createDonationsWithdrawalRequest,
    { isLoading: isCreatingDonationWithdrawalRequest },
  ] = useCreateDonationsWithdrawalRequestMutation();
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    const response = await createDonationsWithdrawalRequest({
      userData,
      access_token,
    });
  ...
  useEffect(() => {
    if (data && amount) {
      if (
        parseFloat(data.funds) + parseFloat(data.pending_withdrawal_donations) <
        amount
      ) {
